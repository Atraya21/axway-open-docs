...
---
{{< alert title="Public beta" color="warning" >}}This is a preview of new Istio discovery agents, which run separately from the current Istio POC agents that provide full governance of your hybrid environment. The new agents are deployed and configured from the Axway CLI, and they allow for updated visibility to the Kubernetes resource discovery process.{{< /alert >}}
